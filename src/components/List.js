...
const List = React.memo(({
    ...
    const handleCompleChange = (id) => {
        let newTodo = todoData.map((data) => {
            if (data.id === id) {
                data.completed = !completed; //not 연산
            }
            return data;
        })
        setTodoData(newTodo);
    ...
    const handleClick = (id) => {
        let newTodoData = todoData.filter(data => data.id !== id); //x버튼을 누른 엘리먼트를 제외한 새로운 요소들
        ...
        return (
            <div className='flex items-center justify-between w-full px-4 py-1 my-2 text-gray-600  bg-gray-100 border rounded'>
                ...
                <div className='items-center'>
                    ...
                    <button className='px-4 py-2 float-right' typy="submit" onClick={handleSubmit}>save</button>
                </div>
            </div >
